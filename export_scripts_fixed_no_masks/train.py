...
@torch.no_grad()
def evaluate(model, loader, device, w_q, w_r, w_u):
    """Evaluation on validation set (fixed dims, unmasked losses)."""
    model.eval()
    total_loss = 0.0
    total_q = 0.0
    total_r = 0.0
    ...
    if n_batches == 0:
        return {"loss": math.nan, "loss_q": math.nan, "loss_r": math.nan, "loss_u": math.nan}
