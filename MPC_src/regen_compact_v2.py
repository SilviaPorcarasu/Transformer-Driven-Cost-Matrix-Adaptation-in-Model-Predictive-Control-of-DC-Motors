"""Compact poster figure: trajectories on top, summary table on bottom."""
import os, sys, csv
...
# Pick a representative scenario (medium / multi_step)
import json as _j
chosen = None
...
# ============ Combined figure: trajectories + table ============
fig = plt.figure(figsize=(13, 11))
gs = GridSpec(2, 3, figure=fig, height_ratios=[1.1, 1.4],
              hspace=0.30, wspace=0.25)

# Top-left: y vs ref
ax_y = fig.add_subplot(gs[0, 0:2])
t = np.arange(len(reference)) * Ts
ax_y.plot(t, reference, "k-", linewidth=2.5, alpha=0.7, label="țintă r(t)")
for name, color, lab in [("baseline", "tab:green", "Referință"),
...
ax_y.set_title("(a) Urmărire țintă — scenariu reprezentativ (mediu, multi-treaptă)",
...
ax_tbl.axis("off")
ax_tbl.set_title("(c) Performanța în buclă închisă pe 80 de scenarii (valori tipice)",
                  fontsize=12, fontweight="bold", pad=10)

headers = ["Metrică", "Referință", "Transformer", "Transformer + MinT"]
data = [
    ...
    ["Energia comenzii",           "153.5",  "174.6",  "88.2"],
    ["Variația comenzii",          "2.59",   "4.36",   "2.41"],
    ["Reziduul Riccati",           "—",      "2011.7", "212.0  (×9.5 ↓)"],
    ["Fezabilitate",               "100 %",  "100 %",  "100 %"],
]
tbl = ax_tbl.table(cellText=data, colLabels=headers, loc="center",
                    cellLoc="center", colLoc="center",
                    colWidths=[0.30, 0.22, 0.22, 0.26])
tbl.auto_set_font_size(False); tbl.set_fontsize(11); tbl.scale(1.0, 1.55)

best_cells = {(1, 2), (2, 1), (2, 2), (3, 3), (4, 3), (5, 3), (6, 3), (7, 3)}
...
fig.suptitle("Rezultate experimentale — performanța în buclă închisă",
